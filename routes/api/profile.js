...
router.get("/user/:user_id" ,async(req,res)=>{
    try {
        ...
        if(!profile) return res.status(400).json({msg:'profile not found in this id'})
        res.json(profile)
        
    } catch (error) {
        console.log(error);
        if(error.kind === 'ObjectId'){
            return res.status(400).json({msg:'profile not found in this id'})
        }
        res.status(500).send("server error")
